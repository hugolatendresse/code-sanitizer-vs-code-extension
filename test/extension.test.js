...
//         assertSomeTokensSame(originalText, clipboardText, sameExpectedTokens);
//
//         // Replace all text in the editor with "hello world"
//         // const text_before_helloworld = document.getText();
//         // printDebugInfo("text_before_priting_hello_world", text_before_helloworld);
...
suite('Python Parser Test Suite', () => {
        ...
        // TODO DELETE ///
                test('print', async () => {
        const {originalTextPythonText1, expectedTokensTest1}  = require('./test_python')
        printDebugInfo("originalTextPythonText1 FINE?", originalTextPythonText1)
        printDebugInfo("expectedTokensTest1 WHERE IS SHOULD BE FINE", expectedTokensTest1);
})
        });
...
//         printDebugInfo("originalTextPythonText1 FINE?", originalTextPythonText1)
//         await editor.edit(editBuilder => {
//             editBuilder.insert(new vscode.Position(0, 0), originalTextPythonText1);
//         });
//
//         // Copy and sanitize
...
//         // printDebugInfo("originalText", originalTextPythonText1);
//         printDebugInfo("expectedTokensTest1 WHERE IS SHOULD BE FINE", expectedTokensTest1);
//         assertSomeTokensSame(originalTextPythonText1, document.getText(), expectedTokensTest1);
//
...
//         const clipboardText = await vscode.env.clipboard.readText();
//         // printDebugInfo("vscode.env.clipboard.readText() line 282 (expect 7 lines of sanitized text)", clipboardText);
//         assertSomeTokensSame(originalTextPythonText1, clipboardText, expectedTokensTest1);
//
//         // Replace all text in the editor with "hello world"
//         // const text_before_helloworld = document.getText();
//         // printDebugInfo("text_before_priting_hello_world", text_before_helloworld);
...
//
//         // Replace all text in the editor with the unsanitized text
//         assertAllTokensDifferent(originalTextPythonText1, document.getText());  // Should have nothing in common with Hello World
//         await vscode.commands.executeCommand('editor.action.selectAll');
//         await vscode.commands.executeCommand('code-sanitizer.unanonymizeAndPaste');
//
...
//         const expected = originalTextPythonText1.replace(/\r\n/g, '\n')
...
//
//         // Assert that every token in selection is different from the original text, except for SQL words
//         assertSomeTokensSame(originalText, document.getText(), expectedTokensTest1);
//
//         // Assert that the third line is as before
//         const pastedLines = document.getText().split('\n');
//         assert.strictEqual(pastedLines[2], "import pandas as pd\r");
